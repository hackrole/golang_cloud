...
func (eh *eventServiceHandler) FindEventHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		w.WriteHeader(400)
		fmt.Fprint(w, `{"error": "No search criteria found, you can either search
    by id via /id/4 to search by name via /name/coldplayconcert"}`)
		return
	}
	...
	if !ok {
		w.WriteHeader(400)
		fmt.Fprint(w, `{"error": "NoSearch keys found, you can either search
		by id via /id/4
...
func (eh *eventServiceHandler) AllEventhandler(w http.ResponseWriter, r *http.Request) {
	events, ok := eh.dbHandler.FindAllAvailableEvents()
	if err != nil {
		w.WriteHeader(500)
