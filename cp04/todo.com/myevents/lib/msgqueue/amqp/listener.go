...
func NewAMQPEventEmitter(conn *amqp.Connection, queue string) msgqueue.EventListener {
...
func (a *amqpEventEmitter) Listen(eventNames ...string) (<-chan msgqueue.Event, <-chan error, error) {
	if err != nil {
		return nil, nil, err
	}
	defer channel.Close()
	...
	go func() {
		for msg := range msgs {
			// todo map messages to actual event struct
