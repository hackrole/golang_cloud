...
func NewKafkaEventListener(client sarama.Client, partitions []int32) (msgqueue.EventListner, error){
...
func (k *kafkaEventListener) Listen(events ...string) (<-chan msgqueue.Event, <-chan error, error){
  ...
  log.Printf("topic %s has partitions: %s", topic, partitions)
  ...
  for _, partitions := range partitions{
    ...
    go func() {
      for msg := range con.Messages(){
        ...
        default:
          errors <- fmt.Errorf("unknown event type: %s", body.EventgName)
          continue
        }
